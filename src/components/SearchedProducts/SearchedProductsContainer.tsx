...
const SearchedProductsContainer: React.FC<TSearchContainerProps> = (props) => {
    ...
    const changePageHandler = (event: React.ChangeEvent<unknown>, page: number) => {
        history.replace(`/Search/${page}`)
        SetCurrentPage(page)
    }

    useEffect(() => {
        props.PutCurrentPageProducts(currentPage,"SearchResult")
    }, [currentPage,props.searchResult])
    ...
    return <div className="ProductsContainer">
        ...
        <ProductsPage Products={props.currentPageProducts}
                      ...
                      pagesCount={ Math.ceil(props.searchResult.length / props.portionSize)}/>
...
const MSTP = (state: GlobalState): T_MSTP_SearchContainer => ({
    ...
    searchResult : state.ProductsData.SearchResult,
...
(MSTP, {SearchProducts, FilterProducts, PutCurrentPageProducts})(SearchedProductsContainer))